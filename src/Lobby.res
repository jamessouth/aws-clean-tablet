...
@react.component
let make = (~cognitoUser, ~wsConnected, ~ingame, ~leadertoken, ~games, ~send, ~wsError) => {
...
| false => switch (wsConnected, games->Js.Nullable.toOption) {
...
>{"start a new game"->React.string}</button>
{
    ...
    | false => <GamesList games ingame leadertoken send=sendfunc user=cognitoUser/>
    }
}
</div>
}
